...
To configure an embedded Locator, annotate your Spring `@PeerCacheApplication` or `@CacheServerApplication`
with `@EnableLocator`, as follows:
...
NOTE: Using `@EnableLocator` starts an embedded Locator running on "`localhost`",
...
To configure an embedded Manager, annotate your Spring `@PeerCacheApplication` or `@CacheServerApplication`
with `@EnableManager`, as follows:
...
NOTE: Using `@EnableManager` starts an embedded Manager running on "`localhost`",
listening on the default Manager port, `1099`.
...
To configure the embedded HTTP server, annotate your Spring `@PeerCacheApplication` or `@CacheServerApplication`
with `@EnableHttpService`, as follows:
...
NOTE: Using `@EnableHttpService` starts an embedded HTTP server (Jetty) running on "`localhost`",
...
To configure the embedded Memcached server (Gemcached), annotate your Spring `@PeerCacheApplication`
or `@CacheServerApplication` with `@EnableMemcachedServer`, as follows:
...
NOTE: Using `@EnableMemcachedServer` starts an embedded Memcached server running on "`localhost`",
...
To configure the embedded Redis server, annotate your Spring `@PeerCacheApplication` or `@CacheServerApplication`
with `@EnableRedisServer`, as follows:
...
NOTE: Using `@EnableRedisServer` starts an embedded Redis server running on "`localhost`",
...
NOTE: Default `log-level` is "`config`".
...
which does not require any special configuration out-of-the-box in order to properly identify domain objects
to serialize and perform the serialization since, the logic in `MappingPdxSerializer` is based on Spring Data's
mapping infrastructure. See <<mapping.pdx-serialize>> for more details.
...
To enable security, annotate your Spring, {data-store-name} client or server application class with `@EnableSecurity`,
as follows:
...
configuration annotations, annotate your Spring, {data-store-name} client or server application with
...
NOTE: Some {data-store-name} properties are client-side only while others are server-side only.
...
and have {sdg-acronym} automatically create {data-store-name} Regions for the caches required by your application services,
then annotate your Spring, {data-store-name} client or server application with `@EnableGemfireCaching`
and `@EnableCachingDefinedRegions`, as follows:
...
Then, simply go on to define the application services that required caching, as follows:
